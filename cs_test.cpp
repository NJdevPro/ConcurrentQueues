...
#include <thread>
#include <cassert>
#include "include/ConcurrentStack.h"

using MyStack = ConcurrentStack<int>; // define the type of queue here

unsigned int unique = 0;	// unique item id

void produce(MyStack& q, unsigned int producer, unsigned int iter) {
...
void consume(MyStack& q, unsigned int consumer, unsigned int iter) {
    for (size_t i = 0; i< iter; ++i) {
        auto item = q.pop();
